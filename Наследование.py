...
class Bags_owner(Bags):
    def __init__(self, color, brand, name, age):#Добавили имя и возраст
        ...
        self.age=age
    def info(self):
        print(f"Это {self.color} портфель от бренда {self.brand}, принадлежащий {self.name} с возрастом в {self.age} лет.")#Так компания подтверждает данные пользователя.
B=Bags_owner('салатовый','puma','Василий','29')
B.info()#Итак, в случае пропажи портфеля вы легко сможете его вернуть, предъявив данные компании, чтобы она сверилась с введённые вами ранее!
