...
print(A.info())#Заполнив на сайте его данные, вам чего-то не хватало.
...
class Bags_owner(Bags):
    def __init__(self, color, brand, name, age):#Добавили имя и возраст
        ...
        self.age=age
    def info(self):
        return f"Это {self.color} портфель от бренда {self.brand}, принадлежащий {self.name} с возрастом в {self.age} лет."#Так компания подтверждает данные пользователя, но и злоумышленники смогут так получить данные пользователя.
...
class Bags_owner(Bags):
    ...
    def __info(self):#Благодаря вашим усилиям, вы поможете многим пользователям!
        return f"Это {self.color} портфель от бренда {self.brand}, принадлежащий {self.name} с возрастом в {self.age} лет."

B=Bags_owner('салатовый','puma','Василий','29')
print(B.__info())#В классе "B" пропускается наличие __info и выводится ошибка об отсутствии этого метода. Благодаря этому, данные скрыты! Все спасены!
